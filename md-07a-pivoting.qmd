...
2.  What's the `tidyverse` Package? Describe in maximum two sentences below. **Summarized from Perplexity.ai, the tidyverse() package is an opinionated collection of R packages designed for data science that includes several core packages commonly used in everyday data analyses, such as for data import, tidying, manipulation, visualization, etc.**
...
1.  Starting with the `waste_data_long_mean` object, execute the code and observe the output in the Console. Would you publish this table in this format in a report? **No, I would want the data in a wide format table for inclusion in a report.**
...
2.  Execute the code and observe the output in the Console. Would you publish this table in a report in this format? **Yes, this is a better format, although I would still want to clean up the formatting before putting it in a report.**
